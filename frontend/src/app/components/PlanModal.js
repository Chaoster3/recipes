...
const PlanModal = ({ addToMealPlan, modalChange, favorites }) => {
    ...
    const handleSubmit = () => {
        // Add all selected recipes to the cart
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 9999 }}>
            ...
            {/* Modal */}
            <div 
                className="relative max-w-7xl w-11/12 bg-white rounded-xl shadow-xl transform transition-all max-h-[90vh] flex flex-col"
                style={{ zIndex: 10001 }}
            >
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    ...
                    <div className="text-center">
                        <h2 className="text-2xl font-bold text-gray-800">Select Recipes to Add</h2>
                        {selected.length > 0 && (
                            <p className="text-sm text-orange-600 mt-1">
                                {selected.length} recipe{selected.length === 1 ? '' : 's'} selected
                            </p>
                        )}
                    </div>
                ...
                <div className="flex items-center justify-center gap-3 px-6 py-4 border-t border-gray-100">
                    ...
                    >
                        {selected.length === 0 
                            ...
                            : `Add ${selected.length} Recipe${selected.length === 1 ? '' : 's'} to List`
                        }
                    </button>
                </div>
            </div>
