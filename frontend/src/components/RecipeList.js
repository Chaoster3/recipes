...
const RecipeList = ({ recipes, changeShown }) => {
    ...
    return (
        <div className="max-w-6xl mx-12 px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {recipes.results.map((recipe) => (
                    <div
                        key={recipe.id}
                        className="group bg-white border-0 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 cursor-pointer overflow-hidden border border-gray-100"
                    ...
                    >
                        <div className="relative">
                            ...
                            <div className="absolute top-3 right-3 backdrop-blur-md bg-white/90 px-3 py-1.5 rounded-full text-sm font-semibold flex items-center gap-1.5 shadow-lg z-20">
                                ...
                                <span className="text-gray-800 text-xs">{Math.round(recipe.spoonacularScore / 2) / 10}</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4">
                            ...
                            <div className="flex items-center text-xs text-gray-600 mb-4 space-x-6">
                                <div className="flex items-center bg-orange-50 px-2 py-1.5 rounded-full">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {recipe.diets.map((diet, index) => (
                                    <span
                                        key={`diet-${index}`}
                                        className="px-2 py-1 text-[0.6rem] font-semibold bg-green-100 text-green-700 rounded-full ring-1 ring-green-200"
                                    >
                                        {diet}
                                    </span>
                                ))}
                                {recipe.dishTypes.map((type, index) => (
                                    <span
                                        key={`type-${index}`}
                                        className="px-2 py-1 text-[0.6rem] font-semibold bg-purple-100 text-purple-700 rounded-full ring-1 ring-purple-200"
                                    >
                                        {type}
                                    </span>
                                ))}
                                {recipe.cuisines.map((cuisine, index) => (
                                    <span
                                        key={`cuisine-${index}`}
                                        className="px-2 py-1 text-[0.6rem] font-semibold bg-blue-100 text-blue-700 rounded-full ring-1 ring-blue-200"
                                    >
                                        {cuisine}
                                    </span>
                                ))}
